...
def generate_noise(size,num_samp=1,device='cuda',type='gaussian', scale=1):#产生噪声（默认高斯分布）
    if type == 'gaussian':
...
def adjust_scales2image(real_,opt):#根据预设的scale_factor_init和输入的image计算得到需要训练的scale数量和其他参数，通过opt传递过去；同时对输入图像做个resize
