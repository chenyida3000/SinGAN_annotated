...
def SinGAN_generate(Gs,Zs,reals,NoiseAmp,opt,in_s=None,scale_v=1,scale_h=1,n=0,gen_start_scale=0,num_samples=50):
    ...
    if in_s is None: #检查是否有输入图像
        in_s = torch.full(reals[0].shape, 0, device=opt.device)
    images_cur = []
    for G,Z_opt,noise_amp in zip(Gs,Zs,NoiseAmp): #设置pad_size和噪声图像规格
        ...
        for i in range(0,num_samples,1):
            if n == 0: #n即指当前所在层数，代码中的第0层即为论文中的第N层（即最粗糙层）
            ...
            if images_prev == []:
                ...
                I_prev = images_prev[i]
                I_prev = imresize(I_prev,1/opt.scale_factor, opt)
                if opt.mode != "SR": #对I_prev进行规格修改和上采样
