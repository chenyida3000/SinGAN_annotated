...
#运行代码：python paint2image.py --input_name balloons.png --ref_name birds.png --paint_start_scale 1
...
if __name__ == '__main__':
    ...
    #    print("output already exist")
        ...
        try:
            ...
            pass
        real = functions.read_image(opt) #读入image
        real = functions.adjust_scales2image(real, opt) #根据预设的scale_factor_init和输入的image计算得到需要训练的scale数量和其他参数，通过opt传递过去；同时对输入图像做个resize
        Gs, Zs, reals, NoiseAmp = functions.load_trained_pyramid(opt) #加载训练好的网络
        if (opt.paint_start_scale < 1) | (opt.paint_start_scale > (len(Gs)-1)):
            print("injection scale should be between 1 and %d" % (len(Gs)-1))
        else:
            ref = functions.read_image_dir('%s/%s' % (opt.ref_dir, opt.ref_name), opt) #读入paint
            if ref.shape[3] != real.shape[3]: #对齐原始paint和原始image规格，方便后续下采样操作
                ref = imresize_to_shape(ref, [real.shape[2], real.shape[3]], opt)
                ref = ref[:, :, :real.shape[2], :real.shape[3]]

            N = len(reals) - 1
            n = opt.paint_start_scale
            # 下面几行把ref下采样并裁剪之后，作为in_s输入generate函数，实际上是将降采样后的image作为网络最底层的结果，输入到了N-1层
            in_s = imresize(ref, pow(opt.scale_factor, (N - n + 1)), opt) # 下采样
            in_s = in_s[:, :, :reals[n - 1].shape[2], :reals[n - 1].shape[3]] # 裁剪为最底层图片规格，注意这里n为paint_start_scale（默认为1）
            in_s = imresize(in_s, 1 / opt.scale_factor, opt) # 上采样到N-1层
            in_s = in_s[:, :, :reals[n].shape[2], :reals[n].shape[3]] # 注意这里n为paint_start_scale（默认为1）
